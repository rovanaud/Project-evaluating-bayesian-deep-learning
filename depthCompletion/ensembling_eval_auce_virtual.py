...
for M in M_values:
    coverage_values[M] = {}
    avg_length_values[M] = {}
    coverage_error_values[M] = {}
    abs_coverage_error_values[M] = {}
    neg_coverage_error_values[M] = {}
...
for M_step, M in enumerate(M_values):
    ...
    for run in range(num_runs_per_M[M]):
        ...
        coverage_values[M][run] = np.array(coverage_values_pred)
        avg_length_values[M][run] = np.array(avg_length_values_pred)
        coverage_error_values[M][run] = np.array(coverage_error_values_pred)
        abs_coverage_error_values[M][run] = abs_coverage_error_values_pred
        neg_coverage_error_values[M][run] = neg_coverage_error_values_pred
...
for M in M_values:
    ...
    for value in auc_abs_error_values[M]:
        auc_abs_error_values_mean += value/float(num_runs_per_M[M])

    auc_abs_error_values_var = 0.0
    for value in auc_abs_error_values[M]:
        auc_abs_error_values_var += ((value - auc_abs_error_values_mean)**2)/float(num_runs_per_M[M])
    ...
    for value in auc_neg_error_values[M]:
        auc_neg_error_values_mean += value/float(num_runs_per_M[M])

    auc_neg_error_values_var = 0.0
    for value in auc_neg_error_values[M]:
        auc_neg_error_values_var += ((value - auc_neg_error_values_mean)**2)/float(num_runs_per_M[M])
    ...
    for value in auc_length_values[M]:
        auc_length_values_mean += value/float(num_runs_per_M[M])

    auc_length_values_var = 0.0
    for value in auc_length_values[M]:
        auc_length_values_var += ((value - auc_length_values_mean)**2)/float(num_runs_per_M[M])
...
for M in M_values:
    for run in range(num_runs_per_M[M]):
        if run == 0:
            plt.plot(alphas, np.flip(coverage_values[M][run], 0), color=colors[M], alpha=0.5, label="M = %d" % M)
        else:
            plt.plot(alphas, np.flip(coverage_values[M][run], 0), color=colors[M], alpha=0.5)
plt.legend()
...
for M in M_values:
    for run in range(num_runs_per_M[M]):
        if run == 0:
            plt.plot(alphas, np.flip(avg_length_values[M][run], 0), color=colors[M], alpha=0.5, label="M = %d" % M)
        else:
            plt.plot(alphas, np.flip(avg_length_values[M][run], 0), color=colors[M], alpha=0.5)
plt.legend()
...
for M in M_values:
    for run in range(num_runs_per_M[M]):
        if run == 0:
            plt.plot(alphas, np.flip(coverage_error_values[M][run], 0), color=colors[M], alpha=0.5, label="M = %d" % M)
        else:
            plt.plot(alphas, np.flip(coverage_error_values[M][run], 0), color=colors[M], alpha=0.5)
plt.legend()
...
for M in M_values:
    for run in range(num_runs_per_M[M]):
        if run == 0:
            plt.plot(alphas, np.flip(abs_coverage_error_values[M][run], 0), color=colors[M], alpha=0.5, label="M = %d" % M)
        else:
            plt.plot(alphas, np.flip(abs_coverage_error_values[M][run], 0), color=colors[M], alpha=0.5)
plt.legend()
...
for M in M_values:
    for run in range(num_runs_per_M[M]):
        if run == 0:
            plt.plot(alphas, np.flip(neg_coverage_error_values[M][run], 0), color=colors[M], alpha=0.5, label="M = %d" % M)
        else:
            plt.plot(alphas, np.flip(neg_coverage_error_values[M][run], 0), color=colors[M], alpha=0.5)
plt.legend()
