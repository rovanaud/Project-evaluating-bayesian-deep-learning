...
L = 10 # (number of trained networks)
K = 3 # (number of trails for each network)
...
for M in M_values:
    ...
    for l in range(L):
        network = ToyNet("eval_MC-Dropout-MAP-02-SGD_1_M10", project_dir="./root/evaluating_bdl/toyClassification").cuda()
        network.load_state_dict(torch.load("./root/evaluating_bdl/toyClassification/training_logs/model_MC-Dropout-MAP-02-SGD_1_M10_%d/checkpoints/model_MC-Dropout-MAP-02-SGD_1_M10_epoch_300.pth" % l))
