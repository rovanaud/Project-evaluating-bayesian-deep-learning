...
for M in M_values:
    for iter in range(6):
        ...
        for i in range(M):
            ...
            network.load_state_dict(torch.load("./root/evaluating_bdl/toyClassification/training_logs/model_SGHMC-64_%d/checkpoints/model_SGHMC-64_%d_epoch_%d.pth" % (iter+1, iter+1, int(num_epochs - i*step_size))))
