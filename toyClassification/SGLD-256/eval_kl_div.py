...
for M in M_values:
    ...
    for j in range(7):
        ...
        for i in range(M):
            ...
            network.load_state_dict(torch.load("./root/evaluating_bdl/toyClassification/training_logs/model_SGLD-256_%d/checkpoints/model_SGLD-256_%d_epoch_%d.pth" % (j+1, j+1, int(num_epochs - i*step_size))))
