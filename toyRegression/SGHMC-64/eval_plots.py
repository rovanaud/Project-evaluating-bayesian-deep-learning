...
for M in M_values:
    for iter in range(6):
        ...
        for i in range(M):
            ...
            network = ToyNet("eval_SGHMC-64", project_dir="./root/evaluating_bdl/toyRegression").cuda()
            network.load_state_dict(torch.load("./root/evaluating_bdl/toyRegression/training_logs/model_SGHMC-64_%d/checkpoints/model_SGHMC-64_%d_epoch_%d.pth" % (iter+1, iter+1, int(num_epochs - i*step_size))))
